...
typedef RT_MODEL_q_srv02_lab2_mdl_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * q_srv02_lab2_mdl(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(q_srv02_lab2_mdl_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(q_srv02_lab2_mdl_DW.HILReadAnalogTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: q_srv02_lab2_mdl -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(q_srv02_lab2_mdl_DW.HILReadAnalogTimebase_Task,
        (t_clock) q_srv02_lab2_mdl_P.HILReadAnalogTimebase_Clock, 1000.0, -1)
        ;
      if (result == 0) {
